...
module.exports = options => ({
  ...
  code(input, filepath) {
    ...
    try {
      const _parse = parse(input, absolutePath, Object.assign({}, DEFAULT_OPTIONS, options)),
            ...
            taggedTemplateLocs = _parse.taggedTemplateLocs,
            sourceMap = _parse.sourceMap; // Save `loc` of template literals


      taggedTemplateLocsMap[absolutePath] = taggedTemplateLocs; // Save dummy interpolation lines

      interpolationLinesMap[absolutePath] = interpolationLines; // Save source location

      sourceMapsCorrections[absolutePath] = sourceMap; // Clean saved errors
  ...
  result(stylelintResult, filepath) {
    ...
    !isCausedBySubstitution(warning, lineCorrection[warning.line], interpolationLines)).map(warning => Object.assign({}, warning, {
      ...
      column: getCorrectColumn(taggedTemplateLocs, lineCorrection[warning.line] || warning.line, warning.column)
    }));
